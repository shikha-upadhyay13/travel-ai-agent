...
from app.db.database import save_message as db_save_message, create_booking, get_conversation_history
...
def _save_reply(user_id: str, reply: str):
    """Save assistant reply to both in-memory state and database."""
...
def format_results(mode: str, results: list) -> str:
    if not results:
        return "No results found for your search. Please try different dates or routes."
...
def _perform_search(mode: str, source: str, destination: str, date: str):
    """Execute the search for the given transport mode."""
...
async def chat(request: Request, chat_request: ChatRequest):
    ...
    try:
        ...
        # Track conversation history (in-memory + database)
        add_message(user_id, "user", chat_request.message)
        db_save_message(user_id, "user", chat_request.message, detected_lang)
            ...
            if existing_state.get("awaiting_confirmation"):
                if message.strip().lower() in ["yes", "y", "confirm", "proceed", "ok", "sure", "yep", "yeah", "haan", "ha"]:
                    ...
                    booking_ref = create_booking(
                        user_id, mode, source, destination,
                        resolved_date, results, time_pref
                    )
                    ...
                    # User said something ambiguous during confirmation
        ...
        # -------- SUPPORT (LLM-powered) --------
            ...
            reply = await generate_chat_response(message, history, intent="support")
            _save_reply(user_id, reply)
            return {"reply": reply}
